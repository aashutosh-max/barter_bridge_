...
const CustomNode = ({ data, selected }: any) => selected ? (
  <div className="bg-gray-800/90 backdrop-blur-sm border-2 border-green-500 rounded-xl p-3 text-center shadow-lg z-10 w-[150px]">
    <div className="font-bold text-green-500">{data.name}</div>
    <div className="text-xs text-gray-400">Has: {data.has}</div>
    <div className="text-xs text-gray-400">Wants: {data.wants}</div>
  </div>
) : <div className="w-4 h-4 bg-green-500 rounded-full border-2 border-white shadow-md cursor-pointer" />;

const MapNode = () => <img src="https://staticmap.openstreetmap.de/staticmap.php?bbox=83.0,27.5,85.6,28.5&size=1500x1000&maptype=mapnik" className="w-[1500px] h-[1000px] pointer-events-none select-none" style={{ filter: 'grayscale(100%) invert(90%) contrast(0.9)' }} alt="Map" />;
...
export default function Home() {
  ...
  const loadGraph = async () => {
    ...
    const n: any[] = [];
    if (layout === 'distance') n.push({ id: 'map', type: 'mapNode', position: { x: 0, y: 0 }, data: {}, draggable: false, selectable: false, zIndex: -1 });
    Object.keys(data.users).forEach((id, i) => {
      const u = data.users[id];
      const x = layout === 'distance' ? ((u.lng - 83.0) / 2.6) * 1500 : 250 + Math.cos(i * 2) * 150;
      const y = layout === 'distance' ? ((28.5 - u.lat) / 1.0) * 1000 : 250 + Math.sin(i * 2) * 150;
      n.push({ id, type: 'custom', position: { x, y }, data: { name: u.name, has: u.has_items.join(", "), wants: u.wants_items.join(", ") } });
    });
    setNodes(n);
    setEdges(data.trust_edges.map(([s, t]: any) => ({ id: `e-${s}-${t}`, source: s, target: t, type: "straight", style: { stroke: layout === 'distance' ? '#10b981' : '#374151', opacity: 0.6 } })));
    if (me.name) {
        ...
        {tab === 'my-trades' && (
          <>
            <div className="p-4 flex justify-between items-center border-b border-gray-700 bg-gray-800 z-10">
              <h2 className="text-xl font-bold">My Trade Network ({layout === 'distance' ? 'Distance' : 'Network'} View)</h2>
            ...
            <div className="flex-1 relative overflow-hidden">
              <ReactFlow nodes={nodes} edges={edges} onNodesChange={onNodesChange} onEdgesChange={onEdgesChange} nodeTypes={nodeTypes} fitView minZoom={0.1}>
                {layout !== 'distance' && <Background color="#444" gap={20} />}
                <Controls />
              </ReactFlow>
            </div>
        ...
        {tab === 'settings' && (
          <div className="p-8 max-w-2xl mx-auto w-full">
            ...
            <div className={`${cardCls} flex justify-between items-center mb-4`}>
              <div><h3 className="text-xl font-semibold">Distance Layout</h3><p className="text-gray-400">Group the graph by physical distance.</p></div>
              <button onClick={() => setLayout(layout === 'distance' ? 'network' : 'distance')} className={`w-16 h-8 rounded-full p-1 transition-colors ${layout === 'distance' ? 'bg-green-600' : 'bg-gray-600'}`}>
                <div className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-transform ${layout === 'distance' ? 'translate-x-8' : ''}`}></div>
              </button>
            </div>
          </div>
        )}
